...
def get_object_velocity_size(flow, state, M,ob_flow_only=False): # f,obj_id,im,
    ...
    velocity = 2 * np.sum(flow_object_segmented, axis=(0, 1)) / target_sum
    ...
    if ob_flow_only:
        w1,w2 = np.amax(new_position[:, :, 0]),np.amin(new_position[non_zero_position[:,:,0],0])
        h1,h2 = np.amax(new_position[:, :, 1]),np.amin(new_position[non_zero_position[:,:,1],1])
        return w1,w2,h1,h2
