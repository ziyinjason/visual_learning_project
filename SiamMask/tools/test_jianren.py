...
from tools.fake_pwc_net import fake_pwc_net_flow as load_flow
from tools.fake_pwc_net import load_M

from tools.get_background_flow import get_background_flow as get_back
from tools.obv import get_object_velocity_size,get_homography
from tools.momentum import momentum
from tools.momentum2 import momentum2
from tools.draw_box import draw_bbox
# from tools.fake_pwc_net import fake_pwc_net_flow

from update_velocity import compute_velocity_on_ref
...
parser.add_argument(
    '--mask', action='store_true', help='whether use mask output')
parser.add_argument(
...
parser.add_argument(
    ...
    help='datasets')
parser.add_argument(
    '-l', '--log', default="log_test.txt", type=str, help='log file')
parser.add_argument(
    '-v',
    '--visualization',
    dest='visualization',
...
def track_vot(model, video, hp=None, mask_enable=True, refine_enable=False):
    regions = []  # result and states[1 init / 2 lost / 0 skip]
    image_files, gt = video['image_files'], video['gt']

    # if(video['name']!='drone1'):
    #     return 1, 1

    flow_total = load_flow(video)
    data_set = video['image_files'][0].split('/')[-3]
    # print(data_set)
    M_all=load_M(data_set,video)
    ...
    for f, image_file in enumerate(image_files):
        ...
        if f == start_frame:  # init
            ...
            # ccx, ccy, cw, ch = get_axis_aligned_bbox(gt[f])
            # state['target_pos'] = np.array([ccx, ccy])
            # state['target_sz'] = np.array([cw, ch])
            # pdb.set_trace()
            ...
            # im_bbox=draw_bbox(im,[xtl,ytl,xbr,ybr], color=[255,0,0], width=3)
            ...
            # gt=gt[f-1]
            # # pdb.set_trace()
            # x, y, w, h = cv2.boundingRect((gt).astype(np.uint8))
            # his_velocity = velocity
            # his_sz = state['target_sz']
            # x, y, w, h = get_axis_aligned_bbox(gt[f-1])
            # xbr,ybr= x+w,y+h
            # x,y,xbr,ybr = int(x), int(y), int(xbr), int(ybr)
            # im_bbox=draw_bbox(im,[x,y,xbr,ybr], color=[0,255,255], width=3)

            # gt=gt[f]
            # x, y, w, h = cv2.boundingRect((gt).astype(np.uint8))
            # x, y, w, h = get_axis_aligned_bbox(gt[f])
            # xbr,ybr= x+w,y+h
            # x,y,xbr,ybr = int(x), int(y), int(xbr), int(ybr)
            # im_bbox=draw_bbox(im_bbox,[x,y,xbr,ybr], color=[0,255,0], width=3)
            # im_ref = cv2.imread(video['image_files'][0])
            # mask_ref = state['mask']
            # im_cur = cv2.imread(video['image_files'][3])

            # embed()

            # compute_velocity_on_ref(im_ref, im_cur, mask_ref, state)



            flow = flow_total[f - 1]
            M = M_all[f - 1]
            #new_velocity, new_sz,mean_bg_speed = get_object_velocity_size(flow, state, M)
            # print(mean_bg_speed)
            

            #velocity,state['target_sz'] = momentum(velocity, new_velocity, state['target_sz'],new_sz)
            velocity,new_sz = get_object_velocity_size(flow, state, M, True)


            # state['target_pos'] = [state['target_pos'][0] + velocity[0]+mean_bg_speed[0],
            #                                 state['target_pos'][1] + velocity[1]+mean_bg_speed[1]]
            state['target_sz']=new_sz
            state['target_pos'] = [state['target_pos'][0] + velocity[0],
                                            state['target_pos'][1] + velocity[1]]

            # xtl,ytl= np.round(state['target_pos']-state['target_sz']/2).astype('int')
            # xbr,ybr= np.round(state['target_pos']+state['target_sz']/2).astype('int')
            # im_bbox=draw_bbox(im_bbox,[xtl,ytl,xbr,ybr], color=[0,0,255], width=3)
            # imageio.imwrite('img_vot/bbx_image_'+str(f)+'.jpg',im_bbox)
            # imageio.imwrite('vot_mask/mask_image_'+str(f)+'.jpg',(state['mask'] * 255).astype('uint8'))
            ...
            if b_overlap:
                ...
                # print('In {} frame, his_vel: {}, obj_vel: {}, new_velocity:{}'.format(f, his_velocity, new_velocity, velocity))
                # print('In {} frame, his_sz: {}, obj_sz: {}, new_sz: {}'.format(f, his_sz, new_sz, state['target_sz']))
                regions.append(2)
                lost_times += 1
...
def track_vos(model,
    ...
    # H = flow_total[0].shape[0]
    # W = flow_total[0].shape[1]
    M_all=load_M('DAVIS',video)
    ...
    for obj_id, o_id in enumerate(object_ids):
        ...
        for f, image_file in enumerate(image_files):
            ...
            if f == start_frame:  # init
                ...
                # new_velocity, new_sz,mean_bg_speed = get_object_velocity_size(flow, state, M) #,f,obj_id,im
                # 直接推
                new_velocity,new_sz = get_object_velocity_size(flow, state, M, True)


                # xtl,ytl= int(round(x_min)),int(round(y_min))
                # xbr,ybr= int(round(x_max)),int(round(y_max))
                # im_bbox=draw_bbox(im_bbox,[xtl,ytl,xbr,ybr], color=[255,255,0], width=3)


                
                velocity,state['target_sz'] = momentum2(velocity, new_velocity, state['target_sz'],new_sz)
    ...
    if args.save_mask:
        ...
        pred_mask_final=(np.argmax(pred_mask_final,axis=0).astype('uint8')+1)*(np.max(pred_mask_final,axis=0)>state['p'].seg_thr).astype('uint8')
    ...
    if args.visualization:
        pred_mask_final = np.array(pred_masks)
        pred_mask_final = (
            np.argmax(pred_mask_final, axis=0).astype('uint8') + 1) * (np.max(
                pred_mask_final, axis=0) > state['p'].seg_thr).astype('uint8')
    ...
    for v_id, video in enumerate(dataset.keys(), start=1):
        if vos_enable:
            iou_list, speed = track_vos(
                model, dataset[video],
                cfg['hp'] if 'hp' in cfg.keys() else None, args.mask,
                args.refine, args.dataset in ['DAVIS2017', 'ytb_vos'])
            iou_lists.append(iou_list)
        else:
            lost, speed = track_vot(model, dataset[video],
